...
filename=sprintf('%s%03d.jpg',calib_name,n);
...
plot(x(1,5),x(2,5),'yo','markersize',5); % corner: grid origin, marked yellow
plot(x(1,40),x(2,40),'ro','markersize',5);   % corner
plot(x(1,36),x(2,36),'ro','markersize',5); % corner
hold off;
...
point_b=X_us(:,5);
point_c=X_us(:,40);
point_d=X_us(:,36);             % grid origin, marked yellow
...
eval(['vp=Rc_' num2str(n) '(:,1)']); % X chessboard axis is vertical
